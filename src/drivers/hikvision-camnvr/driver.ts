...
class HikvisionDriver extends Homey.Driver {
    ...
    override async onInit(): Promise<void> { 
        ...
        .registerRunListener(async (args: any, state: any): Promise<boolean> => {     
                    if (args.device) {
    ...
    override async onPair(session: Homey.PairSession): Promise<void> {
        session.setHandler('testConnection', async (data: ConnectionData): Promise<DeviceCallback> => {
			
        const protocol = data.ssl === true ? 'https://' : 'http://';
		
		return new Promise((resolve, reject) => {
			const checkconnect = request({
                url: protocol + data.address + ':' + data.port + '/ISAPI/System/deviceInfo', 
